<%-- 
    Document   : donorMainPane
    Created on : Oct 4, 2018, 10:04:53 AM
    Author     : IICS
--%>
<%@include file="../../include.jsp" %>
<div class="container-fluid">
    <div class="app-title" id="">
        <div class="col-md-5">
            <h1><i class="fa fa-dashboard"></i> Dashboard</h1>
            <p>Together We Save Lives...!</p>
        </div>

        <div>
            <div class="mmmains">
                <div class="wrapper">
                    <ul class="breadcrumbs">
                        <li class="first"><a href="#" class="fa fa-home" onclick="window.location.reload(true);"></a></li>
                        <li><a href="#" onclick="ajaxSubmitData('store/inventoryAndSupplies.htm', 'workpane', 'act=a&i=0&b=a&c=a&d=0&ofst=1&maxR=100&sStr=', 'GET');">Inventory & Supplies</a></li>
                        <li class="last active"><a href="#">Donor Program</a></li>
                    </ul>
                </div>
            </div>
        </div>
    </div>

    <div class="row" id="">
        <div class="col-md-12" id="contentminex">
            <main id="main">
                <input id="tab1" class="tabCheck" type="radio" name="tabs" checked>
                <label class="tabLabels" for="tab1">Donors</label>
                <input id="tab2" class="tabCheck" type="radio" name="tabs">
                <label class="tabLabels" for="tab2">Donations</label>
                <section class="tabContent" id="content1">
                    <div>
                        <div class="row user">
                            <div class="col-md-12" id="moreDonorContent">
                                <div class="row">
                                    <div class="col-md-12" id="BldFac">
                                        <button class="btn btn-primary pull-right" type="button"  id="registerdonor" onclick="registerDonor()" style="float: right"><i class="fa fa-plus-circle"></i>Register Donor</button>
                                    </div>
                                </div>
                                <fieldset>
                                    <%@include file="donors/views/viewDonors.jsp" %>
                                </fieldset>
                            </div>
                        </div>
                    </div>
                </section>
                <section class="tabContent" id="content2">

                </section>
            </main>

            <div class="row">
                <div class="col-md-12">
                    <div id="registerNewDonor" class="modalDialogShelve">
                        <div id="divSection4">
                            <div id="head">
                                <a href="#close" title="Close" class="close2">X</a>
                                <h5 class="modalDialog-title">Register Donor</h5>
                            </div>
                            <div class="row scrollbar" id="content">
                                <div class="col-md-12" id="registerDonorContent">
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<script>
    breadCrumb();
    $('#tableviewdonors').DataTable();
    function registerDonor() {
        window.location = '#registerNewDonor';
        initDialog('modalDialogShelve');
        ajaxSubmitData('internaldonorprogram/registerNewDonor.htm', 'registerDonorContent', 'act=a&i=0&b=a&c=a&d=0&ofst=1&maxR=100&sStr=', 'GET');
    }

    $('#tab2').click(function () {
        ajaxSubmitData('internaldonorprogram/viewDonationsToTransfer.htm', 'content2', 'act=a&i=0&b=a&c=a&d=0&ofst=1&maxR=100&sStr=', 'GET');
    });
</script>