<%-- 
    Document   : viewNodeUnitDetails
    Created on : Aug 27, 2018, 6:20:08 PM
...
<legend> 
        ${model.hyrchObj.hierachylabel} Under ${model.unitObj[3]}: ${model.unitObj[1]}
    </legend>
    <div id="unit-response">
        <div style="float:right">   
            <div id="AddNewRg" class="form-actions text5">
                <button data-toggle="modal" data-target="#addUnit" class="btn btn-primary pull-right" type="button" onClick="ajaxSubmitData('facilityUnitSetting.htm', 'addnew-pane', 'act=a&i=${model.hyrchObj.structureid}&b=${model.b}&c=${model.a}&d=${model.d}&ofst=${model.ofst}&maxR=${model.maxR}&sStr=${model.sStr}', 'GET');"><i class="fa fa-fw fa-lg fa-plus-circle"></i>
        ...
        <c:if test="${model.hyrchObjType=='Unit'}">        
            ...
            <c:if test="${empty model.hyrchObj.facilityunitList}">
                <div align="center">
                    <h3>
                        No Registered ${model.hyrchObj.hierachylabel} 
                        <c:if test="${model.hyrchObj.service==true}">Units</c:if>
                        <c:if test="${model.hyrchObj.service==false}">Unit Level</c:if>
                        </h3>
                    </div>
            </c:if>
        </c:if>  
        ...
        <c:if test="${model.hyrchObjType=='Level'}">        
            ...
            <c:if test="${empty model.hyrchObj.facilityunithierachyList}">
                <div align="center">
                    <h3>
                        No Registered ${model.hyrchObj.hierachylabel} Under ${model.unitObj[3]}: ${model.unitObj[1]}
                        </h3>
                    </div>
            </c:if>
        </c:if>            
