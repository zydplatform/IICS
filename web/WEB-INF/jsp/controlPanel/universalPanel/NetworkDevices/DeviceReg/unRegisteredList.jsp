<%-- 
    Document   : unRegisteredList
    Created on : Mar 14, 2018, 10:31:32 PM
...
<fieldset>
    <legend>Manage Registered Devices On The IICS Network</legend>
<div id="response">
                                ...
                                <tbody>
                                    ...
                                    <c:forEach items="${model.deviceList}" var="list" varStatus="status" begin="0" end="${model.size}">
                                        <c:choose>
                                            ...
                                            <td>${status.count}</td>
                                            <td><fmt:formatDate  type="both" dateStyle="medium" timeStyle="medium" value="${list.daterequested}"/></td>
                                            <td></td>
                                            <td></td>
                                            <td></td>
                                            <td></td>
                                            <security:authorize access="hasRole('ROLE_ROOTADMIN') or hasRole('PRIVILEGE_MANAGEREGISTEREDNETWORKDEVICE')">
                                                <td align="center">
                                                    <a href="#" class="btn btn-xs btn-teal tooltips" data-placement="top"  data-original-title="Edit/Update"  data-toggle="modal" data-target="#panel-grantAccess" onClick="ajaxSubmitData('deviceManuSetting.htm', 'addnew-pane', 'act=c2&i=${list.requestid}&b=${model.b}&c=${model.a}&d=${model.d}&ofst=${model.ofst}&maxR=${model.maxR}&sStr=${model.sStr}', 'GET');"><i class="fa fa-edit"></i></a>
                                                </td>
                                            </security:authorize>
                                            <security:authorize access="hasRole('ROLE_ROOTADMIN') or hasRole('PRIVILEGE_DELETEREGISTEREDNETWORKDEVICE')">
                                                <td align="center">
                                                    <a href="#" data-dismiss="modal" aria-hidden="true" class="btn btn-xs btn-bricky tooltips" data-dismiss="modal" aria-hidden="true" data-placement="top" data-original-title="Remove" onClick="var resp = confirm('Delete Device From Registration Request?');
                                                            if (resp === true) {
                                                                ajaxSubmitData('deviceManuSetting.htm', 'addnew-pane', 'act=e2&i=${list.requestid}&b=${model.b}&c=${model.a}&d=${model.d}&ofst=${model.ofst}&maxR=${model.maxR}&sStr=${model.sStr}', 'GET');
                                ...
                                <div class="PagerContainer" align="center">
                                    <div>
                                        <div>
                                            ...
                                            <img src="static/images/pager/previous.gif" width="16" height="16" alt="Previous Page" onclick="if(document.getElementById('pages').value==1){return false;} movePage(-1,'a'); ajaxSubmitData('deviceManuSetting.htm','panel_overview', 'act=${model.act}&i=${model.i}&b=${model.b}&c=${model.c}&d=${model.d}&ofst='+$('#pages').val()+'&maxR='+$('#maxResults').val()+'', 'GET');" />
                                            <img src="static/images/pager/next.gif" width="16" height="16" alt="Next Page" onclick="if(document.getElementById('pages').value==${model.totalPage}){return false;} movePage(1,'a'); ajaxSubmitData('deviceManuSetting.htm','panel_overview', 'act=${model.act}&i=${model.i}&b=${model.b}&c=${model.c}&d=${model.d}&ofst='+$('#pages').val()+'&maxR='+$('#maxResults').val()+'', 'GET');" />
                                            <img src="static/images/pager/last.gif" width="16" height="16" alt="Last Page" onclick="movePage(document.getElementById('lastPg').value,'b'); ajaxSubmitData('deviceManuSetting.htm','panel_overview', 'act=${model.act}&i=${model.i}&b=${model.b}&c=${model.c}&d=${model.d}&ofst='+$('#pages').val()+'&maxR='+$('#maxResults').val()+'', 'GET');" />
                                        </div>
                                        <div>
                                            <c:set var="nC" value="1"></c:set>
                                            Batches <input type="text" style="width:40px" id="pages" onKeyPress="return isNumberKey(event)" name="pages" value="${model.ofst}" onChange="if(this.value>${model.totalPage}){this.value=${model.totalPage}}"/>of ${model.totalPage}
    ...
    <c:if test="${empty model.deviceList}">
        <div align="center"><h3>No ${model.facilityType}</h3></div>
    </c:if>
</fieldset>
