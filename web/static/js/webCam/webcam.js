...
window.webcam = {
    ...
    get_html: function(width, height, server_width, server_height) {
        ...
        this.loaded = false;
        return document.getElementById("frameContainer").innerHTML = html;
    //   el.innerHTML=html;
    },
    ...
    set_shutter_sound: function(enabled, url) {
        ...
        this.shutter_url = url ? url : 'static/js/webCam/shutter.mp3';
    },
...
function LoadWebMovie(){
    webcam.set_api_url( "photoProcess.htm" );
    webcam.set_quality( 90 ); // JPEG quality (1 – 100)
    webcam.set_shutter_sound( true ); // play shutter click sound
    webcam.set_swf_url( "static/js/webCam/webcam.swf" );
    webcam.set_hook( 'onComplete', 'webcamComplete' );
    webcam.set_hook('onError', 'webcamError');
    $('#frameContainer').add(webcam.get_html(220, 217));
    //document.getElementById('frameContainer').add( webcam.get_html(220, 217) );
}

function webcamComplete(response) {
    alert('Success! Returned: ' + response);
}

function webcamError(error) {
    alert('Error: ' + error);
    //----Divs at updating a staff details---------
//    if(document.getElementById('confirmDetailsResponse')!='undefined' && document.getElementById('confirmDetailsResponse')!=null){
//        hideDiv('updatePhotosDiv');
//        showDiv('updateMaritalAndVillageDiv');
//    }
}